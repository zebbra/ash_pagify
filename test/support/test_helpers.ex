defmodule AshPagify.TestHelpers do
  @moduledoc """
  ...
  ## Example
      ...
      iex> url_a = "/posts?offset=20&limit=10"
      iex> url_b = "/posts?limit=10&offset=20"
      iex> assert_urls_match(url_a, url_b)
      true
  """
  def assert_urls_match(url_a, url_b) when is_binary(url_a) and is_binary(url_b) do
  ...
  ## Example
      ...
      iex> url = "/posts?offset=20&limit=10"
      iex> query = [limit: 10, offset: 20]
      iex> assert_urls_match(url, "/posts", query)
      true
  """
  def assert_urls_match(url, path, query) when is_binary(url) and is_binary(path) and is_list(query) do
    uri = URI.parse(url)
