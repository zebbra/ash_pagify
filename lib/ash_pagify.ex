defmodule AshPagify do
  @moduledoc """
  AshPagify is an Elixir library designed to easily apply full-text search, scoping, filtering,
  ...
  filter forms for your LiveView with the `AshPagify.FilterForm` module.
  ...
  - **Full-text search**: AshPagify supports full-text search using the `tsvector` column in
    PostgreSQL.
  ...
  - **UI helpers and URL builders**: AshPagify provides a `AshPagify.Meta` struct with information about
    the current page, total pages, and more. This information can be used to build pagination links
    in your UI. Further, `AshPagify` provides the `AshPagify.Components` module with headless table and
    pagination components to easily build sortable tables and pagination links in your Phoenix LiveView.
    The `AshPagify.FilterForm` module provides a simple way to build filter forms for your LiveView.
  ...
  To replace invalid ash_pagify parameters with their default values, you can use the `replace_invalid_params?`
  ...
  defp global_option(key) when is_atom(key) do
    :ash_pagify
    |> Application.get_env(:ash_pagify, [])
    |> Keyword.get(key, nil)
  end
end
